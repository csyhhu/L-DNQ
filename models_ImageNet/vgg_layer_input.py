...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000, init_weights=False):
        ...
        self.layer_kernel = dict()
        self.layer_stride = dict()

        self.features = features
        for layer_idx, layer in enumerate(self.features):
            if isinstance(layer, nn.Conv2d):
                # print('Convolution: %d' % layer_idx)
                self.layer_kernel['features.%d' %layer_idx] = 3
                self.layer_stride['features.%d' %layer_idx] =1
            # elif isinstance(layer, nn.BatchNorm2d):
                # print('Batch Normalization: %d' %layer_idx)
